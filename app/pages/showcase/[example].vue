<template>
    <ClientOnly>
      <div class="min-h-screen bg-gray-50 dark:bg-gray-950 select-none" :class="{ 'select-none': isDragging }">
    <!-- Top Navigation -->
      ...
      <div class="max-w-full mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-14">
          <div class="flex items-center gap-3">
            ...
            <div class="flex items-center gap-2">
              ...
              <div class="hidden sm:block">
                <h1 class="text-sm font-bold text-gray-900 dark:text-gray-100">Nuxt UI Forms Showcase</h1>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2">
            ...
            >
              <ClientOnly>
                <UIcon v-if="isDark" name="i-heroicons-sun" class="w-4 h-4 text-yellow-500" />
                <UIcon v-else name="i-heroicons-moon" class="w-4 h-4 text-gray-600" />
                <template #fallback>
                  <UIcon name="i-heroicons-moon" class="w-4 h-4 text-gray-600" />
                </template>
              </ClientOnly>
            </button>

            <div v-if="currentExample" class="hidden lg:flex items-center gap-1.5 px-2 py-1 bg-primary-50 dark:bg-primary-900/20 rounded-lg">
              <UIcon :name="currentExample.icon" class="w-3.5 h-3.5 text-primary-600 dark:text-primary-400" />
    ...
    <div class="flex relative">
      ...
      <!-- Main Content -->
      <main class="flex-1 lg:ml-0 min-h-[calc(100vh-3.5rem)] overflow-hidden" :class="sidebarCollapsed ? 'lg:ml-16' : 'lg:ml-56'">
        <div class="h-[calc(100vh-3.5rem)] flex overflow-hidden">
          ...
          <div class="flex flex-col min-w-0 overflow-hidden" :style="{ width: splitPercent + '%' }">
            <div class="h-full flex flex-col bg-white/80 dark:bg-gray-900/80 backdrop-blur-xl border-r border-gray-200/50 dark:border-gray-700/50">
              ...
              <div class="flex items-center justify-between px-5 py-3 border-b border-gray-200/50 dark:border-gray-800/50 shrink-0">
                ...
                <UBadge v-if="currentExample" color="primary" variant="outline" class="text-[10px] shrink-0 ml-2">
              ...
              <div class="flex-1 overflow-auto p-5">
                <div v-if="liveSchema && liveUischema" :key="'form-' + currentExample?.id" class="max-w-3xl mx-auto">
                  <JsonFormsNuxt
                    :schema="liveSchema"
                ...
                <div v-else class="flex items-center justify-center h-full">
                  <div class="text-center py-12">
                    <UIcon name="i-heroicons-cursor-arrow-rays" class="w-12 h-12 mx-auto text-gray-300 dark:text-gray-600 mb-3" />
                    <h3 class="text-base font-semibold text-gray-900 dark:text-gray-100 mb-1">Bir Örnek Seçin</h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400 max-w-xs mx-auto">Sol menüden seçim yaparak formu görün.</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <!-- Playground Panel -->
          <div class="flex flex-col min-w-0 overflow-hidden" :style="{ width: (100 - splitPercent) + '%' }">
            <PlaygroundPanel
    ...
    <!-- Mobile Bottom Sheet -->
    <div v-if="isMobile && selectedExampleId" class="lg:hidden fixed bottom-0 left-0 right-0 z-40 max-h-[50vh] bg-white/95 dark:bg-gray-950/95 backdrop-blur-xl border-t border-gray-200/50 dark:border-gray-800/50 rounded-t-2xl shadow-2xl">
      <div class="flex items-center justify-between px-4 py-2.5 border-b border-gray-200/50 dark:border-gray-800/50">
        <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100">Playground</h3>
        <button @click="selectedExampleId = ''" class="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-800">
...
const formData = ref<any>(initialEx ? structuredClone(initialEx.data) : null)
const liveSchema = ref<any>(initialEx ? structuredClone(initialEx.schema) : null)
const liveUischema = ref<any>(initialEx ? structuredClone(initialEx.uischema) : null)
const isMobile = ref(false)
const schemaVersion = ref(0)

const splitPercent = ref(process.client ? Number(localStorage.getItem('showcase-split') ?? 70) : 70)
