...
const getSelectedExample = () => {
  return examples.find(e => e.id === props.modelValue)
}
  ...
  >
    ...
    <div class="flex items-center justify-between h-12 px-3 border-b border-gray-200/50 dark:border-gray-800/50 shrink-0">
      <div v-if="!collapsed" class="flex items-center gap-2 min-w-0">
        <div class="w-6 h-6 rounded-lg bg-gradient-to-br from-primary-500 to-primary-600 flex items-center justify-center shrink-0">
          <UIcon name="i-heroicons-code-bracket" class="w-3.5 h-3.5 text-white" />
        </div>
        <div class="min-w-0">
          <h1 class="text-sm font-bold text-gray-900 dark:text-gray-100 truncate">Showcase</h1>
        </div>
      </div>
      <button
      ...
      >
        <UIcon :name="collapsed ? 'i-heroicons-chevron-right' : 'i-heroicons-chevron-left'" class="w-5 h-5 text-gray-600 dark:text-gray-300" />
      </button>
    </div>

    <!-- Navigation + Search -->
    <nav ref="navRef" class="flex-1 overflow-y-auto" aria-label="Ana navigasyon">
      <!-- Sticky Search -->
      <div v-if="!collapsed" class="sticky top-0 z-10 bg-white/80 dark:bg-gray-950/80 backdrop-blur-xl px-2 pt-2 pb-1 border-b border-gray-200/50 dark:border-gray-800/50">
        <UInput
          v-model="searchQuery"
          placeholder="Ara..."
          size="xs"
        ...
        >
          <template #leading>
            <UIcon name="i-heroicons-magnifying-glass" class="w-3 h-3 text-gray-400 dark:text-gray-500" />
          </template>
        </UInput>
      </div>
      ...
      <div class="px-2 py-2">
          ...
          <div class="relative w-full mb-1 last:mb-0">
            ...
            >
              <span class="flex items-center gap-1.5 truncate">
                <UIcon :name="category.icon" class="w-3 h-3 shrink-0" />
            ...
            <!-- Items -->
            <ul v-if="!collapsed && isCategoryExpanded(category.id)" class="relative w-full flex flex-col gap-[1px] mt-[1px]" role="list">
              <li
        ...
        <!-- No results -->
        <div v-if="!collapsed && filteredCategories.length === 0" class="text-center py-8">
          <UIcon name="i-heroicons-magnifying-glass" class="w-6 h-6 mx-auto text-gray-300 dark:text-gray-600 mb-2" />
          <p class="text-xs text-gray-500 dark:text-gray-400">Sonuç bulunamadı</p>
        </div>
      </div>
    </nav>
    ...
    <div class="p-2 border-t border-gray-200/50 dark:border-gray-800/50 shrink-0">
      <div v-if="!collapsed" class="text-center">
        <p class="text-[9px] text-gray-400 dark:text-gray-500 truncate">{{ examples.length }} örnek</p>
      </div>
    </div>
  </aside>
</template>
