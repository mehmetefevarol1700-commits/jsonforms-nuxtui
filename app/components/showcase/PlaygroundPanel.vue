...
const safeStringify = (obj: any, spaces = 2): string =>
...
<template>
  <div class="flex flex-col h-full bg-white/80 dark:bg-gray-950/80 backdrop-blur-xl border-l border-gray-200/50 dark:border-gray-800/50">
    ...
    <div class="flex-1 overflow-auto p-4">
      <!-- Form Data Tab -->
      <div v-show="activeTab === 'data'" class="animate-fade-in" role="tabpanel">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">Canlı Form Verisi</h3>
        ...
        <p v-if="Object.keys(props.formData || {}).length === 0" class="mt-4 text-center text-gray-500 dark:text-gray-400 text-sm">
          Form henüz doldurulmadı. Sol menüden bir örnek seçip form alanlarını doldurun.
        </p>
      </div>

      <!-- JSON Schema Tab -->
      <div v-show="activeTab === 'schema'" class="animate-fade-in" role="tabpanel">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">JSON Schema Tanımı</h3>
          <div class="flex items-center gap-2">
            <UButton
              size="xs"
              variant="outline"
        ...
        <p v-if="schemaError" class="mt-1.5 text-xs text-red-500 dark:text-red-400 font-mono">
          {{ schemaError }}
        </p>
        <p v-else-if="!schemaError && schemaText" class="mt-1.5 text-xs text-green-500 dark:text-green-400">
          Geçerli JSON
        </p>
      </div>

      <!-- UI Schema Tab -->
      <div v-show="activeTab === 'uischema'" class="animate-fade-in" role="tabpanel">
        <div class="flex items-center justify-between mb-4">
          <h3 class="text-lg font-semibold text-gray-900 dark:text-gray-100">UI Schema (Layout Tanımı)</h3>
          <div class="flex items-center gap-2">
            <UButton
              size="xs"
              variant="outline"
        ...
        <p v-if="uischemaError" class="mt-1.5 text-xs text-red-500 dark:text-red-400 font-mono">
          {{ uischemaError }}
        </p>
        <p v-else-if="!uischemaError && uischemaText" class="mt-1.5 text-xs text-green-500 dark:text-green-400">
          Geçerli JSON
        </p>
      </div>
    </div>
    ...
    <div class="border-t border-gray-200/50 dark:border-gray-800/50 p-4 bg-gray-50/50 dark:bg-gray-900/50">
      <div class="grid grid-cols-3 gap-4 text-center">
        <div>
          <p class="text-2xl font-bold text-primary-600 dark:text-primary-400">{{ Object.keys(props.formData || {}).length }}</p>
          <p class="text-xs text-gray-500 dark:text-gray-400">Doldurulan Alan</p>
        </div>
        <div>
          <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ example.value?.schema?.required?.length || 0 }}</p>
          <p class="text-xs text-gray-500 dark:text-gray-400">Zorunlu Alan</p>
        </div>
        <div>
          <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ Object.keys(example.value?.schema?.properties || {}).length }}</p>
          <p class="text-xs text-gray-500 dark:text-gray-400">Toplam Alan</p>
        </div>
      </div>
    </div>
  </div>
