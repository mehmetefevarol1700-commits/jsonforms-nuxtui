...
<template>
  <div v-if="control.visible" class="mb-4 last:mb-0 w-full block">
    <label
      :for="inputId"
    ...
    >
      <div class="min-w-0 flex-1 pr-3">
        ...
        <p v-if="control.description" class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ control.description }}</p>
      </div>
      <div class="flex items-center gap-2 shrink-0">
        <span v-if="control.errors" class="text-xs text-red-500">{{ control.errors }}</span>
        <USwitch
          :id="inputId"
